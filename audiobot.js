...
slack.on('message', function(message) {
    ...
    if(message.type === 'message') {
        ...
        if(messageText) {
            ...
            var julDictionnary = { 'wesh': 'wesh_alors',
                                   'alors': 'wesh_alors',
                                   'mal': 'tout_va_mal_dans_sa_tete',
                                   'nostalgique': 'nostalgique',
                                   'ose': 'tu_oses_demander',
                                   'oser': 'tu_oses_demander',
                                   'meuf': 'refrain_tchikita',
                                   'femme': 'refrain_tchikita',
                                   'bonne': 'delicieuse',
                                   'mignon': 'je_suis_mignon',
                                   'beau': 'je_suis_mignon',
                                   'venere': 'vnr',
                                   'énervé': 'vnr',
                                   'dépression': 'pas_bien',
            ...
            // recupere le titre du son en fonction du match de l'input
            var julDictionnaryKeys = Object.keys(julDictionnary) //recupere les clés du julDictionnary

            var hasPlay = message.text.indexOf("Jul"); //search for play trigger
            if((hasPlay > -1) && (started === true)) {
                //if message has the word play in then try and play a message

                var toPlay = message.text.substring(hasPlay + 3);
                var splittedUserInput = toPlay.match(/\S+/g) || []
                var mixedArray = julDictionnaryKeys.concat(splittedUserInput)
                var sorted_arr = mixedArray.slice().sort();

                var resultsToReturn = [];
                for (var i = 0; i < sorted_arr.length - 1; i++) {
                    if (sorted_arr[i + 1] == sorted_arr[i]) {
                        resultsToReturn.push(sorted_arr[i]);
                    }
                }
                var randomWord = Math.floor(Math.random() * resultsToReturn.length);

                var julWord = julDictionnary[resultsToReturn[randomWord]];
                console.log(julWord)
                // var toPlayWav = 'sounds/' + julWord + '.wav';
                var toPlayMp3 = 'sounds/' + julWord + '.mp3';

                var julPunchline = ['Jul est en train de kicker ça sale..', 'Aigghhttttttt', "Sisi mon gars sûr..", "La famille c'est Marseille"]

                var randomIndex = Math.floor(Math.random() * julPunchline.length);
                var randomPunchline = julPunchline[randomIndex];
                fs.exists(toPlayMp3,function(existsMp3) { //mp3 version of loop
                  if(existsMp3) {
                      exec(player + outputDevice + ' ' + toPlayMp3);
                      played = 'played';
                      channel.send(randomPunchline);
                      console.log('playing: ' + toPlayMp3);
                  } else {
                    channel.send("Heu j'ai rien sous le coude la");
                  }
                });
